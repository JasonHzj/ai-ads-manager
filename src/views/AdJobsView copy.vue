  ...
  <el-card class="box-card">
    ...
    >
      ...
      <el-table-column prop="account_status" label="账户状态" width="90" align="center">
        <template #default="scope">
          <el-tag :type="scope.row.account_status === 'ENABLED' ? 'success' : 'info'">
            {{ scope.row.account_status === 'ENABLED' ? '启用' : '暂停' }}
          </el-tag>
        </template>
      </el-table-column>
...
import { getAccountsAPI, submitJobsAPI, generateAIContentAPI, saveDraftAPI, requestDeleteJobAPI } from '@/api'
